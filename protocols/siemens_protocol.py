...
class SiemensProtocolHandler(BaseProtocolHandler):
    ...
    def _handle_read_error(self, read_result, addr: str, data_type: str):
        ...
        if is_network_error:
            logger.error(f"西门子PLC网络通信失败 {self.device.name} ({error_context}): {full_error_msg}")
            # 存储网络错误到InfluxDB
            try:
            ...
            # 非网络错误，可能是地址错误或权限问题
            logger.warning(f"西门子PLC数据读取失败但PLC在线 {self.device.name} ({error_context}): {full_error_msg}")
            # 存储读取错误到InfluxDB
            try:
                from database import db_manager
                db_manager.write_communication_error(
    ...
    def update_timeouts(self, connect_timeout: int, receive_timeout: int):
        ...
        try:
            ...
            logger.info(f"更新西门子PLC超时配置 {self.device.name}: 连接超时={connect_timeout}ms, 接收超时={receive_timeout}ms")
        except Exception as e:
            logger.error(f"更新西门子PLC超时配置失败 {self.device.name}: {e}")